...
impl LNode {
    ...
    fn to_replacement_node(&self, old_children: &OldChildren) -> ReplacementNode {
        match old_children.0.get(self) {
            ...
            None => match self {
                LNode::Fragment(fragment) => ReplacementNode::Fragment(fragment.iter().map(|node| node.to_replacement_node(old_children)).collect()),
                LNode::Element { name, attrs, children } => ReplacementNode::Element {
                    name: name.to_owned(),
                    attrs: attrs
                        .iter()
                        .filter_map(|(name, value)| match value {
                            LAttributeValue::Boolean => Some((name.to_owned(), name.to_owned())),
                            LAttributeValue::Static(value) => Some((name.to_owned(), value.to_owned())),
                            _ => None,
                        })
                        .collect(),
                    children: children.iter().map(|node| node.to_replacement_node(old_children)).collect(),
                },
                LNode::Text(_) | LNode::Component { .. } | LNode::DynChild(_) => ReplacementNode::Html(self.to_html()),
            },
        }
    }
    ...
    fn diff_at(&self, other: &LNode, path: &[usize], orig_children: &OldChildren) -> Vec<Patch> {
        if std::mem::discriminant(self) != std::mem::discriminant(other) {
            return vec![Patch {
                path: path.to_owned(),
                action: PatchAction::ReplaceWith(other.to_replacement_node(orig_children)),
            }];
        }
        match (self, other) {
            // fragment: diff children
            (LNode::Fragment(old), LNode::Fragment(new)) => LNode::diff_children(path, old, new, orig_children),
            ...
            (
                ...
                let children_patch = LNode::diff_children(path, old_children, new_children, orig_children);
            ...
            (
                ...
                path.push(0);
                path.push(0);
                LNode::diff_children(&path, old_children, new_children, orig_children)
            }
            _ => vec![],
        }
    }
    ...
    fn diff_attrs<'a>(
        ...
        let additions = new
            ...
            .filter_map(|(name, new_value)| {
                ...
                if replace {
                    match &new_value {
                        LAttributeValue::Boolean => Some((name.to_owned(), "".to_string())),
                        LAttributeValue::Static(s) => Some((name.to_owned(), s.to_owned())),
                        _ => None,
                    }
                } else {
                    None
    ...
    fn diff_children(path: &[usize], old: &[LNode], new: &[LNode], old_children: &OldChildren) -> Vec<Patch> {
        if old.is_empty() && new.is_empty() {
            vec![]
        } else if old.is_empty() {
            vec![Patch {
                path: path.to_owned(),
                action: PatchAction::AppendChildren(new.iter().map(LNode::to_html).map(ReplacementNode::Html).collect()),
            }]
            ...
            // diffing in middle
            if b >= a {
                let old_slice_end = if b >= old.len() { old.len() - 1 } else { b };
                let new_slice_end = if b >= new.len() { new.len() - 1 } else { b };
                ...
                for (new_idx, new_node) in new.iter().enumerate() {
                    match old.get(new_idx) {
                        Some(old_node) => {
                            ...
                            let diffs = old_node.diff_at(new_node, &new_path, old_children);
                        ...
                        None => patches.push(Patch {
                            ...
                            action: PatchAction::InsertChild {
                                before: new_idx,
                                child: new_node.to_replacement_node(old_children),
                            },
                        }),
                    }
                }
...
pub enum PatchAction {
    ...
    InsertChild { before: usize, child: ReplacementNode },
    InsertChildAfter { after: usize, child: ReplacementNode },
}
...
mod tests {
    ...
    fn patches_attrs() {
        ...
        assert_eq!(
            ...
            vec![
                Patch {
                    path: vec![],
                    action: PatchAction::SetAttribute("class".into(), "a b".into())
                },
                Patch {
                    path: vec![],
                    action: PatchAction::SetAttribute("id".into(), "button".into())
                },
                Patch {
                    path: vec![],
    ...
    fn patches_child_text() {
        let a = LNode::Element {
            name: "button".into(),
            attrs: vec![],
            children: vec![LNode::Text("foo".into()), LNode::Text("bar".into())],
        };
        let b = LNode::Element {
            name: "button".into(),
            attrs: vec![],
            children: vec![LNode::Text("foo".into()), LNode::Text("baz".into())],
        };
        let delta = a.diff(&b);
        assert_eq!(
            delta,
