...
// the state of a Glory view macro template. This is because
        ...
        match node {
            ...
            Node::Element(el) => {
                if is_component_node(&el) {
                    ...
                    views.push(LNode::Component {
                        ...
                        props: el
                            ...
                            .filter_map(|attr| match attr {
                                NodeAttribute::Attribute(attr) => Some((attr.key.to_string(), format!("{:#?}", attr.value()))),
                                _ => None,
                            })
                            .collect(),
                        children,
                    ...
                    for attr in el.open_tag.attributes {
                        if let NodeAttribute::Attribute(attr) = attr {
                            ...
                            if let Some(value) = attr.value_literal_string() {
                                attrs.push((name, LAttributeValue::Static(value)));
                            } else {
                    ...
                    views.push(LNode::Element { name, attrs, children });
                }
            }
            _ => {}
        }
    ...
    pub fn to_html(&self) -> String {
        match self {
            ...
            LNode::DynChild(_) => "<!--<DynChild>--><pre>Dynamic content will \
                ...
                .to_string(),
            LNode::Element { name, attrs, children } => {
                ...
                let attrs = attrs
                    ...
                    .filter_map(|(name, value)| match value {
                        ...
                        LAttributeValue::Static(value) => Some(format!("{name}=\"{value}\" ")),
                        LAttributeValue::Dynamic => None,
                        LAttributeValue::Noop => None,
                    })
                    .collect::<String>();

                let children = children.iter().map(LNode::to_html).collect::<String>();
