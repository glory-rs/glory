  ...
  try {
    ...
    for (const [id, patches] of views) {
      ...
      const walker = document.createTreeWalker(document.body, NodeFilter.SHOW_COMMENT),
        open = `glory-view|${id}|open`,
        close = `glory-view|${id}|close`;
      let start, end;
      const instances = [];
  ...
  function buildActualChildren(element, range) {
    ...
    while (walker.nextNode()) {
      if (walker.currentNode.nodeType == Node.ELEMENT_NODE) {
        ...
        if (walker.currentNode.textContent.trim().startsWith("glory-view")) {
          if (walker.currentNode.textContent.trim().endsWith("-children|open")) {
            const startingName = walker.currentNode.textContent.trim();
            const componentName = startingName.replace("-children|open").replace("glory-view|");
            const endingName = `glory-view|${componentName}-children|close`;
