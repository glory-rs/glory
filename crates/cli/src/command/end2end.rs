...
pub async fn end2end_proj(proj: &Arc<Project>) -> Result<()> {
    if let Some(e2e) = &proj.end2end {
        ...
        try_run(&e2e.cmd, &e2e.dir).await.context(format!("running: {}", &e2e.cmd))?;
        Interrupt::request_shutdown().await;
        server.await.dot()??;
    } else {
        log::info!("end2end the Crate.toml package.metadata.glory.end2end_cmd parameter not set")
    }
    Ok(())
}
...
async fn try_run(cmd: &str, dir: &Utf8Path) -> Result<()> {
    ...
    let exe = parts.next().ok_or_else(|| anyhow!("Invalid command {cmd:?}"))?;
