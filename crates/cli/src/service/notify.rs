...
pub async fn spawn(proj: &Arc<Project>) -> Result<JoinHandle<()>> {
    ...
    log::info!("Notify watching paths {}", GRAY.paint(paths.iter().join(", ")));
    let proj = proj.clone();
...
fn handle(watched: Watched, proj: Arc<Project>) {
    ...
    if lib_rs || lib_js {
        log::debug!("Notify lib source change {}", GRAY.paint(watched.to_string()));
        changes.push(Change::LibSource);
    }

    if path.starts_with_any(&proj.bin.src_paths) && path.is_ext_any(&["rs"]) {
        log::debug!("Notify bin source change {}", GRAY.paint(watched.to_string()));
    ...
    if !changes.is_empty() {
        ...
        log::trace!("Notify changed but not watched: {}", GRAY.paint(watched.to_string()));
    }
}
...
fn convert(p: &Path, proj: &Project) -> Result<Utf8PathBuf> {
    let p = Utf8PathBuf::from_path_buf(p.to_path_buf()).map_err(|e| anyhow!("Could not convert to a Utf8PathBuf: {e:?}"))?;
