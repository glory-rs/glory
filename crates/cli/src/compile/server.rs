...
pub async fn server(proj: &Arc<Project>, changes: &ChangeSet) -> JoinHandle<Result<Outcome<Product>>> {
    let proj = proj.clone();
    ...
    tokio::spawn(async move {
        ...
        match wait_interruptible("Cargo", process, Interrupt::subscribe_any()).await? {
            CommandResult::Success(_) => {
                ...
                let changed = proj.site.did_external_file_change(&proj.bin.exe_file).await.dot()?;
                if changed {
...
pub fn build_cargo_server_cmd(cmd: &str, proj: &Project, command: &mut Command) -> (String, String) {
    let mut args = vec![cmd.to_string(), format!("--package={}", proj.bin.name.as_str())];
    if cmd != "test" {
        args.push(format!("--bin={}", proj.bin.target))
    }
    args.push(format!("--target-dir={}", proj.bin.target_dir.as_deref().unwrap_or("target/server")));
