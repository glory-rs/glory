...
pub async fn compile_tailwind(_proj: &Project, tw_conf: &TailwindConfig) -> Result<Outcome<String>> {
    ...
    match wait_piped_interruptible("Tailwind", process, Interrupt::subscribe_any()).await? {
        CommandResult::Success(output) => {
            let done = output.stderr().lines().last().map(|l| l.contains("Done")).unwrap_or(false);
