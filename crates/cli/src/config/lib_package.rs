...
impl LibPackage {
    pub fn resolve(cli: &Opts, metadata: &Metadata, project: &ProjectDefinition, config: &ProjectConfig) -> Result<Self> {
        ...
        let profile = Profile::new(cli.release, &config.lib_profile_release, &config.lib_profile_dev);
        ...
        let wasm_file = {
            let source = metadata
                ...
                // .join(name.replace('-', "_"))
                .with_extension("wasm");
            let site = config.site_pkg_dir.join(&output_name).with_extension("wasm");
