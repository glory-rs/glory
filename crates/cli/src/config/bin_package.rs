...
impl BinPackage {
    pub fn resolve(cli: &Opts, metadata: &Metadata, project: &ProjectDefinition, config: &ProjectConfig) -> Result<Self> {
        ...
        let targets = package.targets.iter().filter(|t| t.is_bin()).collect::<Vec<&Target>>();
        ...
        let profile = Profile::new(cli.release, &config.bin_profile_release, &config.bin_profile_dev);

        let exe_file = {
            let file_ext = if cfg!(target_os = "windows") { "exe" } else { "" };
            let mut file = config
                .bin_target_dir
                .as_ref()
                .map(|dir| dir.into())
...
impl std::fmt::Debug for BinPackage {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("BinPackage")
            ...
            .field(
                "src_paths",
                &self.src_paths.iter().map(|p| p.test_string()).collect::<Vec<_>>().join(", "),
            )
...
fn many_targets_found(pkg: &str) -> Error {
    anyhow!(r#"Several bin targets found for member "{pkg}", please specify which one to use with: [[workspace.metadata.glory]] bin-target = "name""#)
}
fn target_not_found(target: &str) -> Error {
    anyhow!(r#"Could not find the target specified: [[workspace.metadata.glory]] bin-target = "{target}""#,)
}
