...
impl PathBufExt for Utf8PathBuf {
    ...
    fn is_ext_any(&self, of: &[&str]) -> bool {
        let Some(ext) = self.extension() else { return false };
        of.contains(&ext)
    }
    ...
    fn ls_ascii(&self, indent: usize) -> Result<String> {
        ...
        out.push(format!("{}{}:", "  ".repeat(indent), self.file_name().unwrap_or_default()));
        ...
        for file in files {
            out.push(format!("{}{}", "  ".repeat(indent), file.file_name().unwrap_or_default()));
        }
