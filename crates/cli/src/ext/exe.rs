...
pub const ENV_VAR_GLORY_SASS_VERSION: &str = "GLORY_SASS_VERSION";
pub const ENV_VAR_GLORY_WASM_OPT_VERSION: &str = "GLORY_WASM_OPT_VERSION";
...
impl ExeMeta {
    ...
    async fn _with_cache_dir(&self, cache_dir: &Path) -> Result<PathBuf> {
        ...
        let c = ExeCache { meta: self, exe_dir };
        c.get().await
    }
...
impl<'a> ExeCache<'a> {
    ...
    async fn fetch_archive(&self) -> Result<Bytes> {
        log::debug!("Install downloading {} {}", self.meta.name, GRAY.paint(&self.meta.url));
    ...
    fn extract_downloaded(&self, data: &Bytes) -> Result<()> {
        ...
        log::debug!("Install decompressing {} {}", self.meta.name, GRAY.paint(self.exe_dir.to_string_lossy()));
    ...
    fn write_binary(&self, data: &Bytes) -> Result<()> {
        ...
        file.write_all(data).context(format!("Error writing binary file: {:?}", path))?;
...
impl Exe {
    pub async fn get(&self) -> Result<PathBuf> {
        ...
        let path = if let Some(path) = meta.from_global_path() {
            ...
            bail!(
                "{} is required but was not found. Please install it using your OS's tool of choice",
                &meta.name
            );
        } else {
        ...
        log::debug!("Command using {} {} {}", &meta.name, &meta.version, GRAY.paint(path.to_string_lossy()));
...
impl Command for CommandWasmOpt {
    ...
    fn env_var_version_name(&self) -> &'static str {
        ENV_VAR_GLORY_WASM_OPT_VERSION
    }
    fn github_owner(&self) -> &'static str {
        "WebAssembly"
    }
...
impl Command for CommandCargoGenerate {
    ...
    fn env_var_version_name(&self) -> &'static str {
        ENV_VAR_GLORY_CARGO_GENERATE_VERSION
    }
    fn github_owner(&self) -> &'static str {
        "cargo-generate"
    }
...
trait Command {
    ...
    /// the `cargo glory` command to see the OS-specific location.
    ///
    /// # Arguments
    ///
            ...
            Ok(dir) => {
                ...
                return match (marker.exists(), marker.is_dir()) {
                    (_, true) => {
                        ...
                        log::warn!(
                            "Command [{}] encountered a conflicting dir in the cache, please delete {}",
                            self.name(),
                            marker.display()
                        );
                    ...
                    (true, _) => {
                        ...
                        if let Some(timestamp) = contents.ok().map(|s| s.parse::<u64>().ok().unwrap_or_default()) {
