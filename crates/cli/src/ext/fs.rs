...
async fn try_rm_dir_content<P: AsRef<Path>>(dir: P) -> Result<()> {
    ...
    if !dir.exists() {
        log::debug!("Glory not cleaning {dir:?} because it does not exist");
        return Ok(());
    }
...
pub async fn read(path: impl AsRef<Path>) -> Result<Vec<u8>> {
    fs::read(&path).await.context(format!("Could not read {:?}", path.as_ref()))
}

pub async fn create_dir(path: impl AsRef<Path>) -> Result<()> {
    log::trace!("FS create_dir {:?}", path.as_ref());
    fs::create_dir(&path).await.context(format!("Could not create dir {:?}", path.as_ref()))
}

pub async fn create_dir_all<P: AsRef<Path>>(path: P) -> Result<()> {
    log::trace!("FS create_dir_all {:?}", path.as_ref());
    fs::create_dir_all(&path).await.context(format!("Could not create {:?}", path.as_ref()))
}
...
pub async fn read_dir<P: AsRef<Path>>(path: P) -> Result<ReadDir> {
    fs::read_dir(&path).await.context(format!("Could not read dir {:?}", path.as_ref()))
}

pub async fn rename<P: AsRef<Path>, Q: AsRef<Path>>(from: P, to: Q) -> Result<()> {
    fs::rename(&from, &to)
        .await
        .context(format!("Could not rename from {:?} to {:?}", from.as_ref(), to.as_ref()))
}

pub async fn remove_file<P: AsRef<Path>>(path: P) -> Result<()> {
    fs::remove_file(&path).await.context(format!("Could not remove file {:?}", path.as_ref()))
}

#[allow(dead_code)]
pub async fn remove_dir<P: AsRef<Path>>(path: P) -> Result<()> {
    fs::remove_dir(&path).await.context(format!("Could not remove dir {:?}", path.as_ref()))
}
...
pub async fn copy_dir_all(src: impl AsRef<Utf8Path>, dst: impl AsRef<Path>) -> Result<()> {
    cp_dir_all(&src, &dst)
        .await
        .context(format!("Copy dir recursively from {:?} to {:?}", src.as_ref(), dst.as_ref()))
}
