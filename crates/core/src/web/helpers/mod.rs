...
#[cfg(all(target_arch = "wasm32", feature = "web-csr", debug_assertions))]
...
#[cfg_attr(debug_assertions, instrument(level = "trace", skip_all))]
#[inline(always)]
pub fn request_animation_frame_with_handle(cb: impl FnOnce() + 'static) -> Result<AnimationFrameRequestHandle, JsValue> {
    cfg_if! {
...
#[cfg_attr(debug_assertions, instrument(level = "trace", skip_all))]
#[inline(always)]
pub fn request_idle_callback_with_handle(cb: impl Fn() + 'static) -> Result<IdleCallbackHandle, JsValue> {
    cfg_if! {
      if #[cfg(debug_assertions)] {
        let span = ::tracing::Span::current();
        let cb = move || {
    ...
    fn ric(cb: Box<dyn Fn()>) -> Result<IdleCallbackHandle, JsValue> {
        ...
        window().request_idle_callback(cb.as_ref().unchecked_ref()).map(IdleCallbackHandle)
    }
...
#[cfg(all(target_arch = "wasm32", feature = "web-csr"))]
#[cfg_attr(
  any(debug_assertions, feature = "web-ssr"),
  instrument(level = "trace", skip_all, fields(duration = ?duration))
)]
#[inline(always)]
pub fn set_timeout(cb: impl FnOnce() + 'static, duration: Duration) -> Result<TimeoutHandle, JsValue> {
    cfg_if! {
      if #[cfg(debug_assertions)] {
        let span = ::tracing::Span::current();
        let cb = move || {
    ...
    fn st(cb: JsValue, duration: Duration) -> Result<TimeoutHandle, JsValue> {
        window()
            .set_timeout_with_callback_and_timeout_and_arguments_0(cb.as_ref().unchecked_ref(), duration.as_millis().try_into().unwrap_throw())
            .map(TimeoutHandle)
    }
...
#[cfg(all(target_arch = "wasm32", feature = "web-csr"))]
#[inline(always)]
pub fn set_interval(cb: impl Fn() + 'static, duration: Duration) -> Result<IntervalHandle, JsValue> {
    cfg_if! {
      if #[cfg(debug_assertions)] {
        let span = ::tracing::Span::current();
        let cb = move || {
    ...
    let handle = window()
        .set_interval_with_callback_and_timeout_and_arguments_0(cb.as_ref().unchecked_ref(), duration.as_millis().try_into().unwrap_throw())?;
...
#[inline(always)]
pub fn window_event_listener_untyped(event_name: &str, cb: impl Fn(web_sys::Event) + 'static) {
    cfg_if! {
      if #[cfg(debug_assertions)] {
        let span = ::tracing::Span::current();
        let cb = move |e| {
...
#[cfg(all(target_arch = "wasm32", feature = "web-csr"))]
pub fn window_event_listener<E: EventDescriptor + 'static>(event: E, cb: impl Fn(E::EventType) + 'static)
where
    E::EventType: JsCast,
{
    window_event_listener_untyped(&event.name(), move |e| cb(e.unchecked_into::<E::EventType>()));
}

#[cfg(all(target_arch = "wasm32", feature = "web-csr"))]
#[doc(hidden)]
/// This exists only to enable type inference on event listeners when in SSR mode.
pub fn ssr_event_listener<E: events::EventDescriptor + 'static>(_event: E, _event_handler: impl FnMut(E::EventType) + 'static) {}
...
#[cfg(all(target_arch = "wasm32", feature = "web-csr"))]
pub fn nodes_between(first: &Node, last: &Node) -> Vec<web_sys::Node> {
    let mut nodes = vec![];
    let mut current = Some(<web_sys::Element as AsRef<web_sys::Node>>::as_ref(first).clone());
