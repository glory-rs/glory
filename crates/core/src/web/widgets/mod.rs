...
#[macro_export]
macro_rules! generate_web_tags {
    ($(
        #[$meta:meta]
        $tag:ident $name:ident $inner:ident $($void:ident)?
    ),* $(,)?) => {
        paste::paste! {
            $(
                #[cfg(all(target_arch = "wasm32", feature = "web-csr"))]
                ...
                pub fn $tag() -> $name {
                    ...
                    $crate::web::widgets::Element::new(stringify!($tag), generate_web_tags!{ @void $($void)? }, inner)
                }
                #[cfg(not(all(target_arch = "wasm32", feature = "web-csr")))]
                pub fn $tag() -> $crate::web::widgets::Element {
                    $crate::web::widgets::Element::new(stringify!($tag), generate_web_tags!{ @void $($void)? })
                }
             )*
        }
    };
