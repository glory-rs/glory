...
//! backend. The `shell` feature adds a `blitz-shell` + vello window entrypoint
//! that routes Blitz click/input events back into Glory's `CommandHolder`.
...
#[cfg(feature = "shell")]
pub use blitz_consumer::launch_blitz_window;
