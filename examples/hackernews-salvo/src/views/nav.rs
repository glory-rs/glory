    ...
    fn build(&mut self, ctx: &mut Scope) {
        header().class("header").fill(
            nav().class("inner").fill(
                a().attr("href", "/home").fill(strong().html("Home"))
            ).fill(
                a().attr("href", "/new").fill(strong().html("New"))
            ).fill(
                a().attr("href", "/show").fill(strong().html("Show"))
            ).fill(
                a().attr("href", "/ask").fill(strong().html("Ask"))
            ).fill(
                a().attr("href", "/job").fill(strong().html("Jobs"))
            ).fill(
                a().class("github").attr("href", "/job").attr("target", "_blank").attr("rel", "noreferrer").html("Built with Leptos")
            )
        ).show_in(ctx);
    }
}
